...
from models.bertApc import BertApc
from processors.apc_seq import convert_examples_to_features
from processors.apc_seq import ner_processors as processors
from processors.apc_seq import collate_fn
from processors.utils_ner import EmotionEntityLib
from tools.finetuning_argparse import get_argparse
MODEL_CLASSES = {
    ## bert ernie bert_wwm bert_wwwm_ext
    'bert': (BertConfig, BertApc, BertTokenizer),
}
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            inputs = {"input_ids": batch[0], "token_type_ids":batch[2], "attention_mask": batch[1],
                      "input_POS_ids": batch[3], "ap_class_labels": batch[4]}
...
def evaluate(args, model, eval_dataset, prefix):
    ...
    for step, batch in enumerate(eval_dataloader):
        ...
        nb_eval_steps += 1
        out_label_ids = inputs['ap_class_labels'].cpu().numpy()
...
def load_and_cache_examples(args, task, tokenizer, data_type='train'):
    ...
    dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids,
                            all_lens, all_POS_ids, all_decode_mask, all_ap_class_ids)
    return dataset
...
def main():
    ...
    label_list = processor.get_ap_labels()
    ...
    config.ap_class_map = ap_class_map
