...
class IDCNN(nn.Module):
    ...
    """
    def __init__(self, input_size, seq_len, filters, kernel_size=3, num_block=4):
        ...
        self.layers = [
            ...
            {"dilation": 2}]
        net = nn.Sequential()
        norms_1 = nn.ModuleList([nn.LayerNorm(seq_len) for _ in range(len(self.layers))])
        norms_2 = nn.ModuleList([nn.LayerNorm(seq_len) for _ in range(num_block)])
