...
def convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        for wid, (word, label) in enumerate(zip(example.words, example.labels)):
            ...
            if len(word_tokens) > 0:
                ...
                if user_X_label:
                    label_ids.extend([label_map.get(label, pad_token_label_id)] + [label_map["X"]] * (len(word_tokens) - 1))
                else:
                    label_ids.extend([label_map.get(label, pad_token_label_id)] + [pad_token_label_id] * (len(word_tokens) - 1))

            if flag and token_position <= max_seq_length - 2 and (word_position == len(example.labels) - 1 or  example.labels[word_position+1] == "O"):
                end = token_position
                if label.split("-")[-1] in label_no_state_map:
                ...
                if line.startswith("#\tpassage"):
                    ...
                    for j, label in enumerate(cols[1:]):
                        ...
                        if label in ["B-Prod", "I-Prod"]:
                            ...
                            if len(predictions) > example_id+j:
                                if len(predictions[example_id+j]) == 0:
                                    output_line.append("O")
                                else:
                                    pred_out = predictions[example_id+j].pop(0)
                                    if type(pred_out) == list:
                                        if len(pred_out) == 0:
                                            output_line.append("O")
                                        else:
                                            output_line.extend(pred_out)
                                    else:
                                        if pred_out == "X":
                                            pred_out = "O"
                                        output_line.append(pred_out)
                            else:
                                logger.info(
