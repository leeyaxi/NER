...
from chemrxnextractor.models import BertForTagging, BertCRFForTagging, BertGlobalPointerForTagging
...
def train(model_args, data_args, train_args):
    ...
    if model_args.use_crf:
        ...
        model = BertGlobalPointerForTagging.from_pretrained(
            model_args.model_name_or_path,
            config=config,
            cache_dir=model_args.cache_dir,
            tagging_schema="BIO",
    ...
    if train_args.do_eval:
        ...
        trainer.log_metrics("eval", output.metrics)
        trainer.save_metrics("eval", output.metrics)
...
def predict(model_args, predict_args):
    ...
    with open(predict_args.input_file, "r") as f:
        ...
        for inputs in tqdm(data_loader, desc="Predicting"):
            ...
            if model_args.use_gp:
                predictions = model.decode(logits, inputs['decoder_mask'].bool())
                preds_list = []
                for sample in predictions:
                    token_pred_id = [[] for i in range(sample.shape[-1])]
                    for l, start, end in zip(*np.where(sample > 0)):
